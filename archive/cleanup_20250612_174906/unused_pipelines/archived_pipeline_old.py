...
class DiscordMessagePipeline:
    ...
    def run_full_pipeline(self, 
        ...
        # Stage 1.5: Resource Detection and Classification
        log.info("Stage 1.5: Detecting and classifying resources...")
        ...
        # Stage 3: Update canonical index
        if save_index:
            log.info("Stage 3: Updating canonical index...")
            save_start = datetime.now()
            
            # Use canonical index builder for consistent indexing
            from scripts.build_canonical_index import CanonicalIndexBuilder
            builder = CanonicalIndexBuilder()
            builder.build_canonical_index(force_rebuild=True)
            
            save_time = (datetime.now() - save_start).total_seconds()
            index_path = "data/indices/discord_messages_index"
            log.info(f"Canonical index updated in {save_time:.1f}s")
...
def main():
    ...
    try:
        ...
        print(f"Resources detected: {stats.get('resources_detected', 0)}")
        print(f"Meetings filtered: {stats.get('meetings_filtered', 0)}")
