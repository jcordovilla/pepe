...
# Description: Enhanced resource sync module with performance optimizations and title enrichment
import os
import json
import logging
import argparse
...
def enrich_resource_title_description(resource_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Uses the enhanced simple_enrich_title function with URL path analysis and domain patterns.
    """
    ...
    if needs_enrichment and url:
        ...
        temp_resource = {
            'url': url,
            'context_snippet': resource_data.get('description', '') or resource_data.get('context_snippet', '') or ''
        }
        ...
        try:
            ...
            logger.debug(f"Enhanced resource: {url} -> '{new_title[:50]}...'")
...
def sync_to_json(output_path: str = "data/resources/detected_resources.json", 
...
def sync_to_json(db_url: str, output_path: str = "data/resources/detected_resources.json"):
    """
    Connect to the database at db_url, fetch all Resource records,
...
if __name__ == "__main__":
    ...
    try:
        if args.format in ['json', 'both']:
            output_path = args.output or "data/resources/detected_resources.json"
            if args.tag:
