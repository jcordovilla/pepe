...
def build_prompt(
    ...
    # 2) USER — only the dynamic context + question
    # Build context lines
    context_lines = []
    for m in matches:
    ...
    if context_lines:
        ...
        header = "Context: (no relevant messages found)"

    # Add JSON instruction if needed
    json_note = "\n\n(Return results as JSON array with fields: guild_id, channel_id, message_id, content, timestamp, author, jump_url.)" if as_json else ""

    user = {
        "role": "user",
        "content": f"{header}\n\nUser's question: {question}{json_note}"
    }
...
def build_resource_prompt(
    ...
    # 2) USER — build clean resource context
    context_lines = []
    for r in resource_matches:
        ...
        # Create clean, compact resource line
        line = f"**{title}** ({tag}) - {domain}"
        line += f"\n📝 {description}"
        line += f"\n👤 {author} | 📊 {score:.3f}"
        if url:
            line += f" | 🔗 {url}"
    ...
    if context_lines:
        ...
        header = "Available Resources: (no relevant resources found)"

    # Add JSON instruction if needed
    json_note = "\n\n(Return as JSON array with: title, description, resource_url, tag, domain, author, similarity_score.)" if as_json else ""
...
def get_hybrid_answer(
    ...
    try:
        ...
        # Build hybrid context with improved system prompt
        system_content = (
        ...
        # Add message context
        if message_matches:
            context_lines.append("## Community Discussions")
            for m in message_matches:
                ...
                url = safe_jump_url(m)
                line = f"**{author_name}** ({ts} in #{ch_name}): {content}"
                if url:
                    line += f" [🔗]({url})"
                context_lines.append(line)
        ...
        if context_lines:
            ...
            header = "Context: (no relevant content found)"

        # Add JSON instruction if needed
        json_note = "\n\n(Return as JSON object with 'messages' and 'resources' arrays.)" if as_json else ""

        chat_messages = [
            {"role": "system", "content": system_content},
            {"role": "user", "content": f"{header}\n\nUser's question: {query}{json_note}"}
        ]
