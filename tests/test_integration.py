...
class TestResourceManagementIntegration:
    ...
    @pytest.fixture
    async def resource_integration_setup(self):
        """Setup for resource integration tests"""
        config = IntegrationTestConfig()
        
        # Create test resource database
        import sqlite3
        resource_db = config.test_data_dir / 'enhanced_resources.db'
        ...
        test_resources = [
            ('https://docs.python.org/3/library/asyncio.html', 'docs.python.org', 
             'Asyncio Documentation', 'Official Python asyncio documentation', 
             9.5, 'excellent', 'documentation', '1001'),
            ('https://github.com/discord-py/discord.py', 'github.com',
             'Discord.py Library', 'Python wrapper for Discord API',
             9.0, 'excellent', 'library', '1003'),
            ('https://realpython.com/async-io-python/', 'realpython.com',
             'Async IO Tutorial', 'Comprehensive asyncio tutorial',
             8.5, 'high', 'tutorial', '1001')
        ]
        
        cursor.executemany('''
            INSERT INTO resources (url, domain, title, description, quality_score, quality_level, category, message_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', test_resources)
        ...
        yield config, resource_db
        
        # Cleanup
        config.cleanup()
    
    async def test_resource_enhanced_search(self, resource_integration_setup):
        """Test search enhanced with resource information"""
        config, resource_db = resource_integration_setup
        
        agent_api = AgentAPI(config.config)
        
        # Add test messages with resource references
        test_messages = [
            {
                'id': '1001',
                'content': 'Check out the official Python asyncio documentation: https://docs.python.org/3/library/asyncio.html',
                'author': 'helpful_dev',
                'channel': 'python-help',
            ...
            {
                'id': '1003',
                'content': 'For Discord bots, use discord.py: https://github.com/discord-py/discord.py',
                'author': 'bot_expert',
        ...
        await agent_api.add_documents(test_messages, source="resource_test")
        
        # Query that should benefit from resource information
        result = await agent_api.query(
            query="Where can I learn about Python asyncio?",
        ...
        assert any(keyword in answer_lower for keyword in ['asyncio', 'documentation', 'python'])
        
        # In a full implementation, this would verify that resource information
        # is properly integrated into the response
    
    async def test_resource_quality_filtering(self, resource_integration_setup):
        """Test resource quality filtering integration"""
        config, resource_db = resource_integration_setup
        ...
        # Query high-quality resources only
        cursor.execute('''
        ...
        high_quality_resources = cursor.fetchall()
        conn.close()
        
        # Should have 2 excellent resources
        assert len(high_quality_resources) == 2
        
        # Verify quality scores
        assert all(resource[5] >= 9.0 for resource in high_quality_resources)  # quality_score column
